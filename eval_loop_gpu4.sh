#!/usr/bin/env bash
# Continuous eval loop for Run B (ckpt400) on GPU 4.
...
CKPT_DIR="logs/20260809-01:56:53-peg_insertion_rl_async_absolute_independent_window_bonus0_fresh_ckpt400_gpu57_notcolocate/peg_insertion_async_ppo_pi05_robometer/checkpoints"
OUT_DIR="logs/20260809-01:56:53-peg_insertion_rl_async_absolute_independent_window_bonus0_fresh_ckpt400_gpu57_notcolocate/peg_insertion_async_ppo_pi05_robometer/rl_eval_sweep_ode_ep50"
...
echo "=== GPU4 continuous eval loop for Run B (ckpt400) ==="
echo "Starting continuous loop."

while true; do
  echo ""
  echo "=== [GPU4] Sweep run at $(date) ==="
  $PY run_train/eval_checkpoint/sweep_peginsertion_wrist.py \
    ...
    --resume \
    --continue-on-error \
    || echo "[GPU4] Sweep exited with error, will retry in 60s"
  echo "[GPU4] Sweep done. Results so far:"
...
" {} \; 2>/dev/null | sort -t' ' -k2 -n
  echo "[GPU4] Sleeping 300s before next check for new checkpoints..."
  sleep 300
done
