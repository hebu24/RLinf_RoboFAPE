#!/usr/bin/env bash
# Parallel PushCube-v1 step20k SFT rollout rendering with human render camera.
#
# Defaults:
...
#
# Videos use ManiSkill's human render camera:
...
# /data/yingxi/robofac has ManiSkill/Torch, but this eval also needs OpenPI + Ray.
# Keep the proven RLinf eval venv by default.
...
OUT_ROOT="${OUT_ROOT:-${REPO_PATH}/logs/pushcube_sft_step20k_human_render_videos_parallel}"
...
BASE_RAY_PORT="${BASE_RAY_PORT:-6480}"
BASE_DASHBOARD_PORT="${BASE_DASHBOARD_PORT:-8365}"
BASE_RAY_WORKER_PORT="${BASE_RAY_WORKER_PORT:-30000}"
...
if [[ ! -x "${VENV_DIR}/bin/python" ]]; then
  echo "Missing Python in VENV_DIR=${VENV_DIR}" >&2
  exit 1
fi
if [[ ! -x "${VENV_DIR}/bin/ray" ]] && ! "${VENV_DIR}/bin/python" -c "import ray" >/dev/null 2>&1; then
  echo "Missing Ray executable/module in VENV_DIR=${VENV_DIR}" >&2
  exit 1
fi
...
echo "BASE_RAY_PORT=${BASE_RAY_PORT}"
...
echo "VIDEO_RENDER_SOURCE=human_render"
echo "VIDEO_RENDER_CAMERA_NAME=render_camera"
echo
...
for seed in "${SEED_ARRAY[@]}"; do
  ...
  ray_tmp_dir="/tmp/rp20kh_${seed}_${ray_port}"
  ...
  (
    ...
    bash run_train/eval_checkpoint/run_pushcube_wrist.sh \
      ...
      "+env.eval.video_cfg.render_source=human_render" \
      "+env.eval.video_cfg.render_camera_name=render_camera" \
      "+env.eval.init_params.human_render_camera_configs={shader_pack:default}" \
