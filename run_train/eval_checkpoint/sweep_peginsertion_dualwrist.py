#!/usr/bin/env python3
# ruff: noqa: I001
"""Evaluate every dual-wrist PegInsertion checkpoint and plot metrics by step."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run dual-wrist (front+back) PegInsertion evaluation for all global_step_* "
    ...
    parser.add_argument(
        "--run-script",
        default=str(REPO_PATH / "run_train/eval_checkpoint/run_peginsertion_dualwrist.sh"),
        help=(
            "Launcher script to invoke per checkpoint. Point this at "
            "run_peginsertion_dualwrist_insert_only.sh for insert-only sweeps."
        ),
    )
    ...
    env.update(
        {
            ...
            "MANAGE_RAY": "false",
            "RAY_TMP_DIR": f"/tmp/ray_eval_dualwrist_{os.getpid()}_{step}",
        }
    )
...
def write_rows(rows: list[dict[str, Any]], output_dir: Path) -> None:
    json_path = output_dir / "dualwrist_sweep_metrics.json"
    json_path.write_text(json.dumps(rows, indent=2) + "\n", encoding="utf-8")

    csv_path = output_dir / "dualwrist_sweep_metrics.csv"
...
def plot_rows(rows: list[dict[str, Any]], output_dir: Path) -> None:
    ...
    fig.tight_layout()
    combined_path = output_dir / "dualwrist_sweep_curves.png"
...
def start_shared_ray(args: argparse.Namespace) -> None:
    ...
    ray_tmp_dir = Path(f"/tmp/ray_eval_dualwrist_sweep_{os.getpid()}")
...
def main() -> None:
    ...
    output_dir = (
        ...
        if args.output_dir
        else REPO_PATH / "logs" / "peginsertion_dualwrist_ckpt_sweep"
    )
