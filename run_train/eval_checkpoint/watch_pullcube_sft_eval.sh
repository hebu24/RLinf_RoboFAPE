...
CHECKPOINT_DIR="${CHECKPOINT_DIR:?Set CHECKPOINT_DIR to the PullCube training checkpoints directory}"
OUTPUT_DIR="${OUTPUT_DIR:-${CHECKPOINT_DIR%/checkpoints}/pullcube_sweep_seed0_7_eval50}"
GPU_IDS="${GPU_IDS:-0,1}"
SEEDS="${SEEDS:-0-7}"
...
RAY_TEMP_DIR="${RAY_TEMP_DIR:-/data/ray_tmp_eval_pullcube_sweep}"
...
exec "${VENV_DIR}/bin/python" run_train/eval_checkpoint/sweep_pushcube_wrist.py \
  --checkpoint-dir "${CHECKPOINT_DIR}" \
  --output-dir "${OUTPUT_DIR}" \
  --run-script run_train/eval_checkpoint/run_pullcube_wrist.sh \
  --watch \
  --venv-dir "${VENV_DIR}" \
  --gpu-ids "${GPU_IDS}" \
  --seeds "${SEEDS}" \
