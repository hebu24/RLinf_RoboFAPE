...
echo "=== 350 eval start $(date) GPU1 RLINF_ROBOFPE_PATH=$RLINF_ROBOFPE_PATH ===" | tee "$LOG"
/data/yingxi/kairan/envs/rlinf/bin/python run_train/eval_checkpoint/sweep_peginsertion_wrist.py \
  ...
  --output-dir "$OUT" \
  --resume \
  --step 50 --step 100 --step 150 --step 200 --step 250 --step 300 --step 350 \
...
echo "=== 350 eval exit=$? $(date) ===" | tee -a "$LOG"
